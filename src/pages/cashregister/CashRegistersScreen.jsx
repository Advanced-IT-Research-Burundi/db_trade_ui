...
const CashRegisterScreen = () => {
  ...
  const loadCashRegisters = async (page = 1) => {
    try {
      setLoading(true);
      const params = { page, ...filters };
      const response = await ApiService.get('/api/cash-registers', params);
      
      if (response.success) {
        setCashRegisters(response.data.cash_registers?.data || []);

        setAgencies(response.data?.agencies || []);
        setPagination({
          current_page: response.data.cash_registers?.current_page,
          last_page: response.data.cash_registers?.last_page,
          total: response.data.cash_registers?.total,
          from: response.data.cash_registers?.from,
          to: response.data.cash_registers?.to
        });
      } else {
