...
const ExpenseTypeCreateScreen = () => {
  ...
  const handleSubmit = async (values) => {
    const response = await ApiService.post('/api/expense-types', values);
    ...
    if (response.success) {
      ...
      toast.current.show({
        severity: 'success',
        summary: 'Succès',
        detail: 'Type de dépense créé avec succès',
        life: 3000
      });
      ...
      return { success: true, message: 'Type de dépense créé avec succès' };
    } else {
      toast.current.show({
        severity: 'error',
        summary: 'Erreur',
        detail: response.message || 'Erreur lors de la création du type de dépense',
        life: 3000
      });
...
export default ExpenseTypeCreateScreen;