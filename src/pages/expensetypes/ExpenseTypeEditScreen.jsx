...
const ExpenseTypeEditScreen = () => {
  ...
  const loadData = async (expenseTypeId) => {
    const response = await ApiService.get(`/api/expense-types/${expenseTypeId}`);
  ...
  const handleSubmit = async (values, isEditing, entityId) => {
    const response = await ApiService.put(`/api/expense-types/${entityId}`, values);
    ...
    if (response.success) {
      ...
      toast.current.show({
        severity: 'success',
        summary: 'Succès',
        detail: 'Type de dépense modifié avec succès',
        life: 3000
      });
      ...
      return { success: true, message: 'Type de dépense modifié avec succès' };
    } else {
      toast.current.show({
        severity: 'error',
        summary: 'Erreur',
        detail: response.message || 'Erreur lors de la modification du type de dépense',
        life: 3000
      });
                ...
                <div className="row">
                  <div className="col-12">
                    <FormField
                      ...
                      placeholder="Saisissez le nom du type de dépense"
                      icon="pi pi-wallet"
                      required
...
export default ExpenseTypeEditScreen;