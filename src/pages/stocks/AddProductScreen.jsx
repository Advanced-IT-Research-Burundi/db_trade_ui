...
const StockTransferScreen = () => {
  ...
  const [validatingProforma, setValidatingProforma] = useState(null);
  ...
  // Nouvelle fonction pour valider un proforma
  const validateProforma = async (proforma) => {
    ...
    try {
      setValidatingProforma(proforma.id);
      
      // Parser les proforma_items (qui est un string JSON)
      let proformaItems = [];
      try {
      ...
      // Récupérer les détails des produits depuis l'API
      const productIds = proformaItems.map(item => item.product_id);
      const response = await ApiService.post('/api/stock-transfers/stocks/products-by-ids', {
      ...
      // Préparer les nouveaux produits à ajouter
      ...
      proformaItems.forEach(item => {
        ...
        if (productFromAPI) {
          ...
          if (!selectedProducts.find(p => p.id === item.product_id)) {
            newSelectedProducts.push(productFromAPI);
            newQuantities[item.product_id] = Math.min(
              item.quantity, 
              productFromAPI.stock_quantity || 0
            );
          } else {
            // Si déjà sélectionné, mettre à jour la quantité
            newQuantities[item.product_id] = Math.min(
              item.quantity, 
              productFromAPI.stock_quantity || 0
            );
      ...
      // Mettre à jour la liste des produits pour exclure les nouveaux produits ajoutés
      updateProductList(formData.selectedCategory);

      showToast('success', `${newSelectedProducts.length} produit(s) ajouté(s) depuis le proforma`);

    } catch (error) {
      showToast('error', 'Erreur lors de la validation du proforma: ' + error.message);
    } finally {
      setValidatingProforma(null);
    }
  };
                          ...
                          <ul className="dropdown-menu">
                            <li>
                              <button 
                                ...
                                disabled={validatingProforma === proforma.id}
                              >
                                {validatingProforma === proforma.id ? (
                                  <>
