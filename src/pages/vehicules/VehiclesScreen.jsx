...
const VehicleScreen = () => {
  ...
  const loadVehicles = async (page = 1) => {
    try {
      ...
      const response = await ApiService.get('/api/vehicules', params);
      
      if (response.success) {
        setVehicles(response.data.vehicles?.data || []);
        setPagination({
          current_page: response.data.vehicles?.current_page,
          last_page: response.data.vehicles?.last_page,
          total: response.data.vehicles?.total,
          from: response.data.vehicles?.from,
          to: response.data.vehicles?.to
        });
      } else {
